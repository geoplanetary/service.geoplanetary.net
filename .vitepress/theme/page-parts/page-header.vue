<template>
	<header>
		...
		<div v-if="willApplicateArticle" class="custom-block info">
			...
			<p>
				この文書は
				<DateTip :date="applicableDate" fuzzyness="minute" /> に発効する版です。
			</p>
		...
		<div v-if="willObsoleteArticle" class="custom-block warning">
			...
			<p>
				この文書は
				<DateTip :date="obsoleteDate" fuzzyness="minute" /> に失効する版です。
			</p>
		...
		<div v-else-if="isObsoletedArticle" class="custom-block danger">
			<p class="custom-block-title">現在表示しているのは過去の版です。</p>
			<p>
				この文書は
				<DateTip :date="obsoleteDate" fuzzyness="minute" /> に失効した過去の版です。
...
const obsoleteDate = computed(() => {
	if (!frontmatter.value.obsolete || typeof frontmatter.value.obsolete !== 'object') { return undefined; }
...
function parseDate(d: unknown): number | undefined {
	if (typeof d === 'number') {
		return d;
