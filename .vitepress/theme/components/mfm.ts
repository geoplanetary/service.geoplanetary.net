...
function transform(node: mfm.MfmNode): string | VNode | (string | VNode)[] {
	switch (node.type) {
		...
		case 'url': {
			...
			return h('a', { href: node.props.url, target: loc.isLocal(node.props.url) ? undefined : '_blank' }, node.props.url);
		}
		case 'link': {
			const loc = useLocation();
			return h('a', { href: node.props.url, target: loc.isLocal(node.props.url) ? undefined : '_blank' }, node.children.flatMap((i) => transform(i)));
		};
